$(document).ready(function() {
    ...
    function deliverCNDeliveryGroup() {
        ...
        if (has_ps_ver_radioBtn.val()) {
            ...
            proceed = confirm("Do you really want to update delivery group " + deliveryGroupNumber + " with product set version " + ps_select_box.val() + " ? ");
        } else {
            productSetVersion = false;
            proceed = confirm("Do you really want to update delivery group " + deliveryGroupId + " without updating product set version? ");
        }
        if(proceed) {
            ...
            $.ajax({
                type: 'PUT',
                url: '/api/cloudNative/deliveryQueue/updateDeliveryGroupByCNProductSetVersion/',
                headers: { "X-CSRFToken": getCookie("csrftoken") },
                dataType: 'json',
                data: JSON.stringify(params),
                success: function(result) {
                    let productName = "Cloud Native ENM"
                    alert("Product set version successfuly updated. ");
                    window.location = "/cloudNative/" + productName + "/deliveryQueue/" + dropNumber + "/" + deliveryGroupNumber + "?section=delivered";
                    $('body').removeClass('loading');
                },
                error: function (xhr, textStatus, errorThrown) {
                    alert("Issue updating product set version: " + xhr.responseText);
                    $('body').removeClass('loading');
