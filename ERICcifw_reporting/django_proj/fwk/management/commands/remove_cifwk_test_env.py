...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        print "killing " +str(options['tplname']) +str(options['appname'])
        command = "/proj/lciadm100/cifwk/latest/bin/vapp_tunnel -t "+str(options['tplname'])+" -a "+str(options['appname'])+ " -f teardown"  
        result = os.system(command)
        if result  == 0:
            logger.info("Successfully deleted vapp")
        else:
            raise CommandError("Could delete instantiate " + options['tplname'] + " as " + options['appname'])
