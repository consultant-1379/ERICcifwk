...
LOGFILE="${LOGDIR}/autodeployment_deployment_description.log"
if [ -f "${LOGFILE}" ]; then
...
# --------------------------------------------
# Run materialise to run in the definition XML:
litp /definition/ materialise

# Enable the MS1 IP address and blade (SND applicable)
litp /inventory/deployment1/network/<NODE_IP_UNSCORE>/ enable
...
# UPDATE NFS Mounts
# ---------------------------------------------
<Append Here File: mountpoint-all>

<Append Here File: jboss-service-grp-instance-all>

# ---------------------------------------------
# VCS Allocation
# ---------------------------------------------
<Append Here File: vcs-command-ms-only>
